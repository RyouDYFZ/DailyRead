...
写作模型只看到事实清单，不直接看到新闻正文；清单列出允许使用的实体、数字、事实和可选引语。事实审校以非阻断告警形式报告潜在问题，不再消耗生成重试或阻止发布。写作提示仍禁止补写行业背景、法规比较、用户规模、预测或因果分析。系统不会因文章略偏离目标词数而直接失败，也不对正文数字执行机械的来源字符串匹配。核心校验仍会检查：字段完整性、词汇类别数量、词性、长难句数量与正文对应关系、题目数量、题目类型、选项结构，以及答案与答案串一致性。
