...
```text
RSS → 抓取正文 → DeepSeek 生成 → 结构校验 → DeepSeek 事实审校
                                                ↓
                                   返回具体问题给 DeepSeek 修订
                                                ↓
                                      通过后写入 Markdown / JSON
...
审校问题会作为修改建议返回模型；系统不再逐字校验长难句，也不会因文章略偏离目标词数而直接失败。核心校验仍会检查：字段完整性、词汇类别数量、词性、长难句数量、题目数量、题目类型、选项结构、答案与答案串一致性，以及来源中不存在的数字。
...
| --- | --- |
| 事实审校提示词 | `audit_facts()` |
| 最大修订轮数 | `generate_with_retries(..., attempts=3)` |
